...
export default function ExercisePage() {
    ...
    return (
        ...
        <Link href={`/gif/${exerciseID}`} className="w-40 h-12 mb-20 mt-8 mx-auto bg-white/80 hover:bg-white/70 focus:bg-slate-200/60 rounded font-bold text-center justify-center flex"><button>GIF</button></Link>
        {newSet.map(set => (
            <div key={set}>
                <Set id={set} />
                {set===0?null: <button onClick={() => deleteSet(set)} className="block w-80 mt-1 rounded-b-lg h-8 mx-auto bg-white/80 hover:bg-white/70 focus:bg-slate-200/60">Delete set</button>}
            </div>
        ))}
        <button onClick={renderNewSet} className="block w-80 h-12 mt-20 mx-auto bg-white/80 hover:bg-white/70 focus:bg-slate-200/60 rounded font-bold">+ Add set</button>
